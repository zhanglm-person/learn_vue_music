...
export default class Player extends Mixins(PlayerMixin) {
  ...
  @Mutation(types.SET_FULLSCREEN) public setFullScreen!: (flag: boolean) => void
  @Mutation(types.SET_PLAYING_STATE) public setPlaying!: (flag: boolean) => void
  ...
  public enter(el: Element, done: () => void) { // 元素，回调（回调执行之后才能进入下一个钩子函数）
  ...
  public updateTime(e: Event) {
    ...
    this.currentTime = (e.target as HTMLAudioElement ).currentTime // 媒体文件的currentTime属性，可读可写
  }

  public onProgessBarChange(percent: number) {
  ...
  public format(interval: number): string {
    // 对当前播放时间和总时长做格式化调整
    const itval = Math.floor(interval) // 向下取整，效果 => Math.floor
    const minute = Math.floor(itval / 60)
    const second = this.pad((itval % 60).toString())
    return minute + ':' + second
  }

  public handleLyric({ lineNum, txt }: { lineNum: number, txt: string }) { // 回调函数在每次歌词改变的时候就会触发
    this.currentLineNum = lineNum
    if (lineNum > 5) { // 当前行数超过5就开始滚动
      const lineEl = this.$refs.lyricLine[lineNum - 5] // 滚动的是到当前元素-5的元素的位置，相对于显示的位置也就是上移一行。
  ...
  public middleTouchEnd() {
    ...
    const el = this.$refs.lyricList.$el as HTMLElement
    el.style[transform] = `translate3d(${offsetWidth}px,0,0)`
    el.style[transitionDuration] = `${durationTime}ms`
  ...
  private pad(num: string, n = 2): string {
    // 补零函数
  ...
  private getLyric() {
    ...
    this.currentSong.getLyric().then((lyric) => {
      ...
      // 利用Lyric创建一个对象,传入歌词字符串和回调函数，回调函数可以获取当前播放的第几条歌词和内容
      if (this.playing) {
